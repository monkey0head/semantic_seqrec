   ...
   # if wandb_logging and accelerator.is_main_process:
            ...
            if do_eval and ((iter_idx + 1) % eval_every == 0 or iter_idx + 1 == start_iter + iterations):
                ...
                if accelerator.is_main_process:
                    ...
                    if improvement > early_stopping_min_delta:
                        ...
                        print(f"New best validation loss: {best_val_loss:.6f} at iteration {iter_idx}")
                    else:
                        # No improvement
                        patience_counter += 1
                        print(f"No improvement for {patience_counter} evaluation cycles. Best loss: {best_val_loss:.6f}")
                        
                        if patience_counter >= early_stopping_patience:
                            print(f"Early stopping triggered after {patience_counter} evaluation cycles without improvement")
    ...
    if accelerator.is_main_process:
        ...
        if best_model_state is not None:
            best_model_file = os.path.join(save_dir_root, f"best_model_iter_{best_iter}.pt")
            torch.save({
                "iter": best_iter,
